...
type
  ...
  (*
    ...
    TCrawlerManager = class(TThreadPoTThreadManagerrManager)
    private
...
(*
  ...
  try
  CrawlerExec(_CrawlerCollectionItem, FComponentController);
  finally
  Free;
  end;
  ...
  CreateTask(TCrawlerThread.Create(AControlController), 'TCrawlerThread (' + AControlController.FindControl(cReleaseName).Value + ')').MonitorWith(FOmniTED).Schedule(FThreadPool);
  end;
