...
uses
  // Delphi
  SysUtils, Variants, XMLDoc, XMLIntf, ActiveX, TypInfo, Rtti,
  // Common
...
type
  ...
  public
    ...
    class procedure SubSearch(ANode: IXMLNode; AComponentController: IComponentController; var AID: Variant);
    class function LoadSettingsToClass(AFileName: TFileName; ASettings: TCMSPlugInSettings; AComponentController: IComponentController = nil;
...
class procedure TPlugInCMSSettingsHelper.SubSearch(ANode: IXMLNode; AComponentController: IComponentController; var AID: Variant);
var
  z: Integer;
  controltype: string;
  control: IBasic;
  ...
  with ANode do
    if HasChildNodes then
      ...
      begin
        ...
        control := AComponentController.FindControl(StringToTComponentID(controltype));
        if Assigned(control) then
          if MatchText(VarToStr(ChildNodes.Nodes[z].Attributes['value']), control.Value) then
          begin
            ...
            TPlugInCMSSettingsHelper.SubSearch(ChildNodes.Nodes[z], AComponentController, AID);
            Break;
          end;
      end;
end;

class function TPlugInCMSSettingsHelper.LoadSettingsToClass(AFileName: TFileName; ASettings: TCMSPlugInSettings;
  AComponentController: IComponentController = nil; AXMLAccess: TXMLAccess = nil): TIntegerArray;
                  ...
                  with ChildNodes.Nodes[rttiProperty.Name] do
                    if HasChildNodes then
                      for I := 0 to ChildNodes.Count - 1 do
                        if SameText(VarToStr(ChildNodes.Nodes[I].Attributes['name']), TTemplateTypeIDToString(AComponentController.TemplateTypeID)) then
                        begin
                          ...
                          TPlugInCMSSettingsHelper.SubSearch(ChildNodes.Nodes[I], AComponentController, IDValue);
                          Break;
                        end;
            end;
