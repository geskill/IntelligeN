...
type
  ...
  protected
    ...
    function AmazonSearchRequest(AWebsite, ASearchAlias, ATitle: string; out AIdHTTPHelper: TIdHTTPHelper): string;
    function AmazonExtractGameCategory(AHTML: string; AInsideHTML: Boolean = False): TTemplateTypeID;
    function AmazonDetailedPageRequest(AIdHTTPHelper: TIdHTTPHelper; AWebsite: string): string;
  public
...
begin
  AIdHTTPHelper := TIdHTTPHelper.Create(Self);
  AIdHTTPHelper.Request.Referer := AWebsite;

  Result := AIdHTTPHelper.Get(AWebsite + 's/ref=nb_sb_noss?url=' + HTTPEncode(ASearchAlias) + '&field-keywords=' + HTTPEncode(ATitle) + '&x=0&y=0'); ;
end;
...
begin
  Result := AIdHTTPHelper.Get(AWebsite);
end;
    ...
    if (AComponentController.FindControl(cRuntime) <> nil) and (cRuntime in _ComponentIDs) then
      with TRegExpr.Create do
        try
          ...
          begin
            Expression := '<dt>Runtime:<\/dt><dd>(\d+) minutes<\/dd>';
  ...
  begin
    with TRegExpr.Create do
      try
        ...
        if Exec(InputString) then
          deep_search(AmazonDetailedPageRequest(IdHTTPHelper, HTMLDecode(Match[1])));
