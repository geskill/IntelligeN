...
if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'add_entry')) {
	$use_account = false;
	if (isset($_POST['account_username']) && ($_POST['account_username'] != '') && isset($_POST['account_password'])) {
		$use_account = true;
	...
	if (isset($_POST['MirrorCount']) && ($_POST['MirrorCount'] > 0)) {
		...
		if (isset($_POST['IType']) && in_array($_POST['IType'], $StringTypeID)) {
			$type = $_POST['IType'];
			$subject = $_POST['ISubject'];

			$values = array();
			foreach ($StringControlID as $value) {
		...
		xmlresult(false, 'No download links given!');
	}
} else {
